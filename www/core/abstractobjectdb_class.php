...
abstract class AbstractObjectDB {
	...
	/*--- получить массив свойст properties ---*/
	...
	/* функция получения суммы числового столбца по заданным условиям */
	public static function getSumOnWhere( $table_name, $field, $where = false, $values = false){
		$select = new Select();
		$select->from( $table_name, array("SUM($field)") );
		if($where){
	...
	protected static function getFieldOnWhere($table_name, $class, $fields, $where = false, $values = false, $order = false, $ask = true, $count = false, $offset = false) {
		$select = new Select();
		...
		elseif ( $order && strtolower($order) != "rand()" ) 
			...
			$select->order("id");
		if ($count) $select->limit($count, $offset);
		$data = self::$db->selectCell($select);
